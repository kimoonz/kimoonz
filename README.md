...
설치가 끝나면 **프로그램 창**이 뜹니다.

```
● 감시 중                                  ← 지금 상태
대상: 2026-09-19  ·  1박  ·  구역 C, D      ← 무엇을 노리는지
120회 확인 · 재고 248칸 중 2칸 예약가능 · 누적 취소 3건

[감시 시작] [설정하기] [지금 조회] [점검]          [로그 지우기]
──────────────────────────────────────────────────
23:39:12  재고 추적 시작
23:39:31  변화 1건: [취소 발생] 2026-09-19 C구역 프리미엄 캐빈
23:39:33  취소표 확보 — 결제 페이지입니다!
```

**[설정하기]** 버튼부터 누르세요. 날짜·박수·구역을 물어본 뒤 브라우저를 띄워
안내대로 클릭만 하면 설정이 끝납니다. 그다음 **[감시 시작]** 을 누르면 됩니다.

다음부터는 `gui.bat`(Windows) 또는 `./gui.sh` 를 실행하면 이 창이 뜹니다.
...
| --- | --- | --- |
| **창으로 쓰기 (권장)** | `gui.bat` 더블클릭 | `./gui.sh` |
| 감시 시작 (창 없이) | `watch.bat` 더블클릭 | `./watch.sh` |
...
paradogo/
  ...
  gui.py        창 화면 — 버튼·상태·로그 (tkinter)
  wizard.py     설치 마법사 — 화면을 관찰해 설정을 자동 생성
...
* 창 화면: 실제 Tk 창을 띄워 [감시 시작] 버튼을 눌러 감시가 시작되고 로그가 흐르며
  재고 248칸을 읽어 상태 표시가 갱신되는 것까지 확인. 로그 상한(2000줄)과
  워커 스레드가 대화상자 답을 기다리는 동작도 확인
