...
> **인코딩 주의**: 저장소의 `.bas` 파일은 UTF-8입니다. VBE의 "파일 가져오기"는 ANSI(한국어 Windows는 CP949)를
> 기대하므로, 한글 주석·프로시저 이름이 깨지면 메모장에서 **다른 이름으로 저장 → 인코딩 ANSI**로 변환한 뒤
> 가져오세요. (또는 파일 내용을 복사해 빈 모듈에 붙여넣어도 됩니다.)
